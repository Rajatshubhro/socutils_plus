...
def get_hso1e_x2c1(mol, unc=True):
    s, t, h1p, x, rp, h1m, xb, rm = get_hxr(mol, uncontract=unc)
    nb = x.shape[0]
...
# ADDED BY RAJAT:
def get_hso1e_x2c2(mol, unc=True):
    ...
    # Construct h1e_2: Eq. 78
    #h1e_2 = numpy.zeros((3, nb, nb))
    #for ic in range(3):
    #    h1e_2[ic] = (c**2) * reduce(numpy.dot, (W1[ic], numpy.linalg.inv(t), O1[ic].T.conj()))
    #    h1e_2[ic] += h1e_2[ic].conj()
    ...
    h1e_sd2 = numpy.zeros((3, nb, nb))
    h1e_sd2[0] = numpy.dot(O1[1],tinv_w1[2]) - numpy.dot(O1[2],tinv_w1[1])
    h1e_sd2[1] = numpy.dot(O1[2],tinv_w1[0]) - numpy.dot(O1[0],tinv_w1[2])
    h1e_sd2[2] = numpy.dot(O1[0],tinv_w1[1]) - numpy.dot(O1[1],tinv_w1[0])
    
    h1e_sd2[0] += numpy.dot(W1[1],tinv_o1[2]) - numpy.dot(W1[2],tinv_o1[1])
    h1e_sd2[1] += numpy.dot(W1[2],tinv_o1[0]) - numpy.dot(W1[0],tinv_o1[2])
    h1e_sd2[2] += numpy.dot(W1[0],tinv_o1[1]) - numpy.dot(W1[1],tinv_o1[0])

    print(numpy.linalg.norm(h1e_sd2[0] + h1e_sd2[0].T.conj()))
    print(numpy.linalg.norm(h1e_sd2[1] + h1e_sd2[1].T.conj()))
    print(numpy.linalg.norm(h1e_sd2[2] + h1e_sd2[2].T.conj()))
...
def get_fso2e_x2c(mol, dm, unc=True):
    ...
    nb = mol.nao_nr()
    s, t, h1, x, rp, h1m, xb, rm = get_hxr(mol, uncontract=unc)
